...
app.post("/plants", (req, res) => {
  const { plantName, description, percentTime } = req.body;
  // instantiate new plant
  const plant = new Plant({ plantName, description, percentTime });
  plant
    .save() //save to db
    .then(doc => {
...
app.put("/plants/", (req, res) => {
  const { _id, plantName, description, percentTime } = req.body;
  console.log(_id);
  console.log(plantName);
  console.log(description);
  console.log(percentTime);
  Plant.findByIdAndUpdate({ _id }, { plantName, description, percentTime })
    .then(doc => {
      res.status(200).json({
        message: "success",
        payload: doc
