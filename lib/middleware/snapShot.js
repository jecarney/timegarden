...
const currentLogReinit = (req, res, next) => {
  CurrentLog.findOneAndUpdate({}, { freeMins: 0 }, { new: true, upsert: true })
    .then(docs => {
      next();
    })
    .catch(err => {
...
const currentProjectsReinit = (req, res, next) => {
  Project.update({}, { absoluteEffortMins: 0 }, { new: true })
    .then(docs => {
      res.status(200).json({
        message: "success",
        payload: docs
...
const snapShotCheck = (req, res, next) => {
  ...
  SnapShot.findOne({ date: yesterday }).then(doc => {
    if (doc) {
      res.status(200).json({
        message: "success"
      });
